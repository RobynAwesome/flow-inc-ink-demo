...
menu?.addEventListener('click',event=>{const header=document.querySelector('.site-header');header?.classList.toggle('open');event.currentTarget.setAttribute('aria-expanded',header?.classList.contains('open')?'true':'false')});
...
contactForm?.addEventListener('submit',event=>{event.preventDefault();if(!requireValid(event.currentTarget))return;const data=new FormData(event.currentTarget);const message=`Hi Flow Inc Ink, I have an enquiry.%0A%0AName: ${encodeURIComponent(data.get('name'))}%0AEmail: ${encodeURIComponent(data.get('email'))}%0AMessage: ${encodeURIComponent(data.get('message'))}`;window.open(`https://wa.me/27606184165?text=${message}`,'_blank','noopener,noreferrer');event.currentTarget.reset()});
if(new URLSearchParams(location.search).get('book')==='1')window.addEventListener('DOMContentLoaded',()=>dialog?.showModal(),{once:true});
if('serviceWorker'in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js').then(registration=>registration.update()).catch(error=>console.info('[Flow PWA] Service worker unavailable.',error)))}
Promise.allSettled([import('/motion-system.js'),import('/experience-system.js'),import('/social-map-system.js')]).then(results=>results.forEach(result=>{if(result.status==='rejected')console.info('[Flow Experience] Progressive enhancement unavailable.',result.reason)}));
